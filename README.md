...
```mermaid
flowchart LR
    A[("🗂️ house-price-dataset\n14,619 records")] 
    A -->|"📊 Tableau"| B["🖥️ Interactive Dashboard"]
    A -->|"🐍 Python"| C["📓 ML Notebook"]
    C --> D["🧹 Data Cleaning"]
    D --> E["📊 EDA"]
...
flowchart TD
    A["📊 14,619 Records
    23 Columns"] -->|"Drop 7 columns"| B["📋 16 Columns
    *15 Features + Target*"]
    B -->|"70/30 split"| C["🎯 Train Set
    ~10,233 rows"]
    B -->|"70/30 split"| D["🧪 Test Set
    ~4,386 rows"]
    C -->|"StandardScaler"| E["Scaled Train"]
    D -->|"StandardScaler"| F["Scaled Test"]
    E --> G["🤖 LinearRegression.fit()"]
    G --> H["🔮 Predict on Test"]
    H --> I{"📊 Evaluation"}
    I -->|"MSE"| J["📉 45,193,596,846"]
    I -->|"R²"| K["✅ 0.6002 ≈ 60%"]
